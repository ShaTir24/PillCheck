...
- Auth: `app/core/security.py`'s `X-Debug-Profile-Id` bypass only works when `SUPABASE_JWT_SECRET` is unset and `ENVIRONMENT=local`. Don't extend that bypass to other environments.
