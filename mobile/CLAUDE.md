...
- **`apiClient` is the only HTTP client.** Don't call `fetch`/`axios` directly from a feature or screen — the bearer-token interceptor (and the 401 refresh-and-retry) only runs through it. The one exception is `src/lib/api/client.ts` itself, which uses bare `axios` for the `/auth/refresh` call to avoid re-entering its own response interceptor.
- **Auth (ADR-006):** `src/store/useAuthStore.ts` persists access/refresh JWTs in `expo-secure-store`; `app/_layout.tsx`'s `useAuthGate` redirects between `/auth/*` and `(tabs)` based on session state. `src/features/auth/api.ts` has the signup/login/forgot/reset hooks.
